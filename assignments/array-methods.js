...
// Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the 
// result
...
// Problem 1
// We want to send everyone that donated more than $100 a box of chocolates. Return an array of runners that have
// donated more than $100. 

let richRunners = runners.filter((item) => {
...
/* Problem 2
 Theres only room for seven characters on the back of the Tshirts at the standards size font. Return an array with all
runners whose last name is more than six characters long so that we can change the font size on their shirt.*/

let longNames = runners.filter((item) => { 
    return item["last_name"].length > 6
});

console.log(longNames)

/* Problem 3
We want to print file folder labels by company name. Return an array of company names alphebetized.
*/
let compNames = runners.map(cname);

function cname(item){
    return item["company_name"];
}

let compNamesSorted = compNames.sort();

console.log(compNamesSorted);
